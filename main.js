 ...
 * Длина круга
 * При панорамировании иногда съезжает текст, разобраться. Похоже, что текст съезжает, если быстро нажать и отпустить space и при этом быстро передвинуть мышь. Проблему это не решает, но хотя бы теперь вроде бы известна причина. Если держать space дольше, текст не съезжает
 * подсказки к кнопкам
 * отмена действий
 * возможность скопировать свойства текста другому тексту
 * и возможность создать объект с такми же свойствами. Это будет относиться не только к тексту.
 * начать внедрять концепцию типа объекта. Для начала это будет тип линии - основная или выноска.
